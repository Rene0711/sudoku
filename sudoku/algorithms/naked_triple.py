...
def check_line(values):
    ...
    for l in line:
        for key, value in values.items():
            if value.value is None and value.line is int(l) and len(value.options) == 3:
...
def check_column(values):
    ...
    for c in column:
        for key, value in values.items():
            if value.value is None and value.column is int(c) and len(value.options) == 3:
                options = value.options
                found_keys = [key]
                for search_key, search_value in values.items():
...
def check_square(values):
    ...
    for s in square:
        for key, value in values.items():
            if value.value is None and value.square is int(s) and len(value.options) == 3:
                options = value.options
                found_keys = [key]
                for search_key, search_value in values.items():
